...
class ExpenseTrackerApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.total_left_label = tk.Label(
            self, text="Expenses Left:", font=("Helvetica", 12)
        )
        self.total_left_label.pack(pady=5)
    ...
    def add_expense(self):
        ...
        try:
            ...
            if category in self.notallowed:
                messagebox.showwarning("Warning", "This category cannot be expensed")
                return
        except ValueError:
            messagebox.showwarning("Warning", "Invalid category")
            return
        
        try:
            amount = float(expense)
            if self.moneyleft.get(category, 0) < amount:
                messagebox.showwarning("Warning", "Cannot spend more than the allotted amount.")
                return
        except ValueError:
            messagebox.showwarning("Warning", "Invalid expense amount.")
            return
        if expense and date:
    ...
    def update_expenses_left(self):
        remaining = self.moneyleft.copy()
        for expense, _, category, _ in self.expenses:
            try:
                ...
                continue  
        remaining_text = "Expenses Left:\n" + "\n".join(
            f"{cat}: USD {amt:.2f}" for cat, amt in remaining.items()
        )
        self.total_left_label.config(text=remaining_text)
